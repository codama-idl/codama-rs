        ...
        meta.as_path_list()?
            .each(|ref meta| match (meta.path_str().as_str(), meta) {
                ...
                (_, Meta::PathList(_) | Meta::Path(_)) => {
                    r#type.set(TypeNode::from_meta(meta)?, meta)
                }
...
mod tests {
    ...
    fn unrecognized_type() {
        assert_node_err!({ fixed_size(unrecognized) }, "unrecognized type");
    }

    #[test]
    fn unrecognized_attribute() {
        assert_node_err!({ fixed_size(foo = 42) }, "unrecognized attribute");
    }
}
